    ...
    listarContactos() {
        if (this.#contactos.length === 0) {
            ...
            this.#contactos.forEach(contacto => {
                console.log(`Nombre: ${contacto.getNombre()}, Teléfono: ${contacto.getTelefono()}`);
                document.write(`Nombre: ${contacto.getNombre()}, Teléfono: ${contacto.getTelefono()}<br>`);
            });
        }
    }
    buscarContacto(nombre) {
        const contacto = this.#contactos.find(c => c.getNombre().toLowerCase() === nombre.toLowerCase());
        if (contacto) {
